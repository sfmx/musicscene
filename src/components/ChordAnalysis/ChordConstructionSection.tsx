    ...
    <div className="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl p-6 sm:p-8 mb-12">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {construction.boxes.map((box, i) => (
          <div
            key={i}
            className="bg-slate-50 dark:bg-slate-950/80 rounded-xl p-5 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-colors"
          >
            ...
            <ul className="text-slate-600 dark:text-slate-300 text-sm space-y-2">
              {box.items.map((item, j) => (
                <li key={j} className="flex items-start gap-2">
                  ...
                  <span><strong className="text-slate-900 dark:text-white">{item.split(':')[0]}:</strong>{item.split(':').slice(1).join(':')}</span>
                </li>
              ))}
            </ul>
          </div>
