    ...
    <Layout>
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 pb-20 transition-colors">
        <main className="max-w-6xl mx-auto px-4 py-8">
          ...
          {/* Hero */}
          {data.heroInfo && (
            <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-100 via-purple-50 to-slate-100 dark:from-slate-900 dark:via-purple-950/40 dark:to-slate-900 border border-purple-200 dark:border-purple-500/30 p-8 mb-12 shadow-sm dark:shadow-2xl">
              <div className="relative z-10">
                {data.heroInfo.title && <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">{data.heroInfo.title}</h2>}
                {data.heroInfo.text && <p className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed mb-4">{data.heroInfo.text}</p>}
                {data.heroInfo.items && (
          ...
          {/* Types Grid */}
          <section className="mb-12">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">Lessons &amp; Categories</h2>
            <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
              {data.types.map((type) => (
                <Link
                  key={type.href}
                  href={type.href}
                  className="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 hover:border-cyan-500/50 hover:bg-slate-50 dark:hover:bg-slate-900/60 transition-all duration-300 flex flex-col justify-between group shadow-sm hover:shadow-md"
                >
                  <div>
                    <div className="flex items-center mb-3">
                      <span className="text-2xl mr-3">{type.icon}</span>
                      <h3 className="text-lg font-bold text-slate-900 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-300 transition-colors">{type.name}</h3>
                    </div>
                    <p className="text-xs text-slate-600 dark:text-slate-400 mb-4 leading-relaxed">{type.description}</p>
                  </div>
                  <div className="pt-3 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between">
                    {type.category && (
                      <span className="px-2 py-0.5 text-[11px] font-mono rounded-full bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400">
                        {type.category}
                      </span>
                    )}
                    <span className="text-xs font-semibold text-cyan-600 dark:text-cyan-400 group-hover:text-cyan-500 dark:group-hover:text-cyan-300 uppercase tracking-wider">
                      Explore &rarr;
                    </span>
                  </div>
                </Link>
