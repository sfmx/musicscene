        ...
        <div className="flex items-start justify-between mb-2">
          <h3 className="text-base font-bold text-slate-900 dark:text-white flex items-center flex-wrap gap-2">
            ...
            {card.number !== undefined && (
              <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-cyan-100 dark:bg-cyan-500/20 text-cyan-800 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-500/30 font-bold text-xs">
                {card.number}
              </span>
            )}
            ...
            {card.fields.map((f, i) => (
              <div key={i}>
                <span className="font-semibold text-slate-500 dark:text-slate-400">
                  <AffiliateLink productName={f.label} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white" />: </span>
                <span className="text-slate-800 dark:text-slate-200"><SmartAffiliateText text={f.value} /></span>
              </div>
            ))}
          </div>
        )}
        ...
        <ul className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-800/80 space-y-1 text-xs text-slate-600 dark:text-slate-300">
          {card.items.map((item, i) => (
            <li key={i} className="flex items-start gap-1.5">
              ...
              <span><AffiliateLink productName={item} className="text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white" /></span>
            </li>
          ))}
        </ul>
      )}
  ...
  return (
    <section className="mb-12">
      <div className="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl p-6 sm:p-8">
        ...
        {section.subtitle && <p className="text-slate-600 dark:text-slate-400 text-sm mb-6 leading-relaxed">{section.subtitle}</p>}
        {section.warningText && (
          <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-500/30 rounded-xl p-4 mb-6">
            <p className="text-amber-800 dark:text-amber-300 text-xs"><strong>⚠️ Note:</strong> {section.warningText}</p>
          </div>
        )}
        {section.layout === 'table' && section.tableHeaders && section.tableRows ? (
          <div className="overflow-x-auto bg-slate-50 dark:bg-slate-950/80 rounded-xl border border-slate-200 dark:border-slate-800">
                  ...
                  <tr key={i} className="hover:bg-slate-100/50 dark:hover:bg-slate-900/50 transition-colors">
                    {row.map((cell, j) => (
                      <td key={j} className="p-3 text-slate-700 dark:text-slate-300">
                        <AffiliateLink productName={cell} className="hover:text-cyan-600 dark:hover:text-cyan-300" />
                      </td>
                    ))}
                  </tr>
                ))}
    ...
    <Layout>
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 pb-20 transition-colors">
        <main className="max-w-4xl mx-auto px-4 py-8">
          ...
          {/* Introduction */}
          <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-100 via-indigo-50/40 to-slate-100 dark:from-slate-900 dark:via-indigo-950/40 dark:to-slate-900 border border-indigo-200 dark:border-indigo-500/30 p-8 mb-12 shadow-sm dark:shadow-2xl">
            <div className="relative z-10">
              {data.introduction.title && <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">{data.introduction.title}</h2>}
              {data.introduction.text.split('\n\n').map((paragraph, i) => (
                <p key={i} className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed mb-4">{paragraph}</p>
              ))}
              {data.introduction.quote && (
                <div className="bg-white dark:bg-slate-950/80 rounded-xl p-4 border-l-4 border-cyan-500 mt-4 shadow-sm dark:shadow-none">
                  <p className="text-slate-700 dark:text-slate-300 italic text-xs leading-relaxed">{data.introduction.quote}</p>
                </div>
              )}
              {data.introduction.columns && data.introduction.columns.length > 0 && (
                <div className={'grid md:grid-cols-' + data.introduction.columns.length + ' gap-4 mt-6'}>
                  {data.introduction.columns.map((col, i) => (
                    <div key={i} className="bg-white/80 dark:bg-slate-950/80 rounded-xl p-4 border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-none">
                      {col.title && (
                      ...
                      <p className="text-slate-600 dark:text-slate-400 text-xs">{col.text}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className={'grid grid-cols-1 ' + (data.relatedTopics.length > 1 ? 'md:grid-cols-2' : '') + ' gap-4'}>
                {data.relatedTopics.map((topic, i) => (
                  <Link
                    key={i}
                    href={topic.href}
                    className="bg-white dark:bg-slate-900/90 rounded-xl p-6 border border-slate-200 dark:border-slate-800 hover:border-cyan-500/50 hover:bg-slate-50 dark:hover:bg-slate-900/60 transition-all group shadow-sm hover:shadow-md"
                  >
                    <h3 className="text-base font-bold text-slate-900 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-300 transition-colors mb-1.5 flex items-center gap-2">
                      {topic.icon && <span>{topic.icon}</span>}
                      {topic.title}
                    </h3>
                    {topic.description && <p className="text-xs text-slate-600 dark:text-slate-400">{topic.description}</p>}
                  </Link>
                ))}
              </div>
            </section>
