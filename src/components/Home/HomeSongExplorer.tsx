...
export default function HomeSongExplorer() {
  ...
  return (
    <section id="song-library" className="py-20 bg-slate-900 text-white border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800 shadow-xl mb-10 space-y-5">
          {/* Live Search Input */}
          <div className="relative">
            <span className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 text-lg">
              🔍
            </span>
            <input
              type="text"
              ...
              className="w-full pl-12 pr-10 py-3.5 bg-slate-900/90 border border-slate-700 rounded-xl text-white placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-all"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-white text-sm"
              >
                ✕
              </button>
            )}
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pt-2 border-t border-slate-800/80">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-xs font-semibold text-slate-400 mr-1">Genre:</span>
        ...
        <div className="flex items-center justify-between mb-6 text-xs text-slate-400">
          <span>
            Showing <strong className="text-white">{displayedSongs.length}</strong> of{' '}
            <strong className="text-white">{filteredSongs.length}</strong> songs
          </span>
        ...
        {displayedSongs.length === 0 ? (
          <div className="bg-slate-950 rounded-2xl p-12 text-center border border-slate-800">
            ...
            <h3 className="text-lg font-bold text-white mb-1">No songs matched your search</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {displayedSongs.map((song: SongListItem) => {
              ...
              return (
                <div
                  key={song.slug}
                  className="bg-slate-950 rounded-2xl p-6 border border-slate-800 hover:border-slate-700 transition-all duration-200 hover:shadow-xl flex flex-col justify-between group"
                >
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-4 text-xs font-mono text-slate-300 bg-slate-900/80 px-3 py-1.5 rounded-lg border border-slate-800/80">
                      <span>🎹 Key: <strong className="text-amber-300 font-bold">{song.key}</strong></span>
                    ...
                    <div className="mt-3.5">
                      {hasCuratedSecret ? (
                        <div className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-md bg-amber-500/10 text-amber-300 border border-amber-500/20">
                          <span>💡</span>
                          <span>Harmonic Secret Breakdown Available</span>
                        </div>
                      ) : song.techniques.length > 0 ? (
                        <div className="text-xs text-slate-400 line-clamp-1">
                          <span className="text-slate-500 font-medium">Techniques: </span>
                  ...
                  {/* Dual Action Links */}
                  <div className="mt-6 pt-4 border-t border-slate-800/80 flex items-center justify-between gap-2 text-xs font-bold">
                    <Link
                      href={`/lessons/songs/song-analysis/${song.slug}`}
                      className="text-white hover:text-amber-400 transition-colors inline-flex items-center gap-1 hover:underline"
                    >
                    ...
                    <Link
                      ...
                      className="text-amber-400 hover:text-amber-300 transition-colors inline-flex items-center gap-1 hover:underline bg-slate-900 px-2.5 py-1 rounded-lg border border-slate-800"
        ...
        {filteredSongs.length > visibleCount && (
          <div className="text-center mt-12">
            <button
              ...
              className="px-8 py-3.5 bg-slate-800 hover:bg-slate-700 text-white font-bold text-sm rounded-xl border border-slate-700 hover:border-slate-600 transition-all shadow-md hover:scale-[1.02] active:scale-[0.98]"
            >
