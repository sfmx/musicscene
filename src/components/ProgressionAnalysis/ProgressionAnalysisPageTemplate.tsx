    ...
    <Layout>
      ...
      <div className="bg-slate-50 dark:bg-slate-950 min-h-screen text-slate-900 dark:text-slate-100 pb-20 transition-colors">
        <main className="max-w-6xl mx-auto px-4 py-8">
          ...
          {/* Hero */}
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-100 via-amber-50/30 to-slate-100 dark:from-slate-900 dark:via-amber-950/30 dark:to-slate-900 border border-amber-200 dark:border-amber-500/30 p-8 mb-12 shadow-sm dark:shadow-2xl">
            <div className="absolute top-0 right-0 w-96 h-96 bg-amber-500/10 rounded-full blur-3xl pointer-events-none" />
            <div className="relative z-10">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-100 dark:bg-amber-500/15 border border-amber-200 dark:border-amber-500/30 text-amber-800 dark:text-amber-300 text-xs font-semibold mb-3">
              ...
              <div className="flex flex-wrap gap-2.5 text-xs font-medium">
                {data.progressionInfo.tags.map((tag, i) => (
                  <span key={i} className="bg-white dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 text-amber-700 dark:text-amber-300 px-3 py-1.5 rounded-lg font-mono">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          <div className="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl p-6 sm:p-8 mb-12">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                <h3 className="text-base font-bold text-cyan-700 dark:text-cyan-300 mb-4">Basic Chord Positions</h3>
                <div className="space-y-4">
                  {data.guitarApplications.chordDiagrams.map((group, i) => (
                    <div key={i} className="bg-slate-50 dark:bg-slate-950/80 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
                      <h4 className="font-bold text-slate-900 dark:text-white mb-2">{group.title}</h4>
                      <p className="text-xs text-slate-600 dark:text-slate-300 mb-3 leading-relaxed">{group.description}</p>
                      <div className={`grid grid-cols-${Math.min(group.chords.length, 3)} gap-3 mb-4`}>
                        {group.chords.map((chord, j) => (
                          <div key={j} className="text-center bg-white dark:bg-slate-900/90 rounded-lg p-3 border border-slate-200 dark:border-slate-800 shadow-sm">
                            <h5 className="font-bold text-xs mb-2 text-slate-900 dark:text-white">{chord.label}</h5>
                            <div className="flex justify-center">
                              <SimpleFretboardDiagram chord={chord.chord} />
                            </div>
                            {chord.subtitle && <p className="text-[11px] text-amber-700 dark:text-amber-400 font-mono mt-1">{chord.subtitle}</p>}
                          </div>
                        ))}
                      </div>
                      {group.alphaTex && (
                      ...
                      {group.practiceNotes && group.practiceNotes.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-800/80">
                          <p className="text-xs text-cyan-700 dark:text-cyan-300 font-semibold mb-1">Practice Notes:</p>
              ...
              <div>
                <h3 className="text-base font-bold text-amber-700 dark:text-amber-300 mb-4">Advanced Applications</h3>
                <div className="space-y-4">
                  {data.guitarApplications.advancedApplications.map((app, i) => (
                    <div key={i} className="bg-slate-50 dark:bg-slate-950/80 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
                      ...
                      <p className="text-xs text-slate-600 dark:text-slate-300 mb-2 leading-relaxed">{app.description}</p>
          ...
          <div className="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl p-6 sm:p-8 mb-12">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                <h3 className="text-base font-bold text-cyan-700 dark:text-cyan-300 mb-4">{data.musicalExamples.left.title}</h3>
                <div className="space-y-4">
              ...
              <div>
                <h3 className="text-base font-bold text-amber-700 dark:text-amber-300 mb-4">{data.musicalExamples.right.title}</h3>
                <div className="space-y-4">
                ...
                {data.notatedExercises.map((ex, i) => (
                  <div key={i} className="bg-slate-50 dark:bg-slate-950/80 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
                    ...
                    {ex.practiceNotes && ex.practiceNotes.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-800/80">
                        <p className="text-xs text-amber-700 dark:text-amber-300 font-semibold mb-1">Practice Tips:</p>
                        <ul className="text-xs text-slate-600 dark:text-slate-400 space-y-1">
          ...
          <div className="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl p-6 sm:p-8 mb-12">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                <h3 className="text-base font-bold text-cyan-700 dark:text-cyan-300 mb-4">{data.scaleRelationships.left.title}</h3>
                {data.scaleRelationships.left.sections.map((section, i) => (
                  <div key={i} className="bg-slate-50 dark:bg-slate-950/80 rounded-xl p-4 border border-slate-200 dark:border-slate-800 mb-4">
                    <h4 className="font-bold text-slate-900 dark:text-white mb-2 text-sm">{section.title}</h4>
              ...
              <div>
                <h3 className="text-base font-bold text-amber-700 dark:text-amber-300 mb-4">{data.scaleRelationships.right.title}</h3>
