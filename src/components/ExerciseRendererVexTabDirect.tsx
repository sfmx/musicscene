...
// A direct VexTab renderer that uses the VexTabDiv global object
const ExerciseRendererVexTabDirect = ({ exercise, width = 900, height = 350 }: ExerciseRendererProps) => {
  const containerRef = useRef<HTMLDivElement>(null);  useEffect(() => {
    ...
    try {
      ...
      // Trigger auto-render if VexTabDiv is available
      if (typeof window !== 'undefined' && window.VexTabDiv && window.VexTabDiv.autoRender) {
        ...
        setTimeout(() => {
          try {
            ...
            console.error("Error in VexTabDiv.autoRender:", err);
          }
        }, 0);
      }
    } catch (error) {
...
export default ExerciseRendererVexTabDirect;
