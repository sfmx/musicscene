      ...
      <div className="grid md:grid-cols-3 gap-6">
        {intervals.map((interval, i) => (
          <div key={i} className="bg-slate-50 dark:bg-slate-950/80 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
            <h3 className="font-bold text-slate-900 dark:text-white mb-2 text-sm">{interval.title}</h3>
            <p className="text-xs text-slate-600 dark:text-slate-300 mb-2">{interval.description}</p>
            <p className="text-xs text-cyan-700 dark:text-cyan-300 font-mono">{interval.detail}</p>
          </div>
        ))}
      </div>
    </div>
