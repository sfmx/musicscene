...
export default function IntervalGuitarSection({ guitarApplications }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl p-6 sm:p-8 mb-12">
      ...
      {/* Interval Examples */}
      <div className="mb-10">
        <h3 className="text-lg font-bold text-cyan-700 dark:text-cyan-300 mb-2">{intervalExamples.title}</h3>
        <p className="text-xs text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">{intervalExamples.description}</p>
          ...
          {intervalExamples.examples.map((ex, i) => (
            <div key={i} className="bg-slate-50 dark:bg-slate-950/80 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
              <div className="text-center mb-4">
                <h4 className="text-base font-bold text-slate-900 dark:text-white mb-1">{ex.title}</h4>
                <p className="text-xs text-slate-600 dark:text-slate-400 mb-3">{ex.subtitle}</p>
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div className="text-center bg-white dark:bg-slate-900/90 rounded-lg p-3 border border-slate-200 dark:border-slate-800 shadow-sm">
                    <div className="flex justify-center">
                      <SimpleFretboardDiagram chord={ex.chord1} />
                    </div>
                    <p className="text-xs font-mono text-cyan-700 dark:text-cyan-300 mt-1">{ex.chord1Label}</p>
                  </div>
                  <div className="text-center bg-white dark:bg-slate-900/90 rounded-lg p-3 border border-slate-200 dark:border-slate-800 shadow-sm">
                    <div className="flex justify-center">
      ...
      {chordConstructions && chordConstructions.items && chordConstructions.items.length > 0 && (
        <div>
          <h3 className="text-lg font-bold text-amber-700 dark:text-amber-300 mb-6">{chordConstructions.title}</h3>
          <div className="grid lg:grid-cols-3 gap-6">
            {chordConstructions.items.map((item, i) => (
              <div key={i} className="bg-slate-50 dark:bg-slate-950/80 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
                <h4 className="text-base font-bold text-slate-900 dark:text-white mb-4 text-center">{item.title}</h4>
                <div className="space-y-4">
                  <div className="text-center bg-white dark:bg-slate-900/90 rounded-lg p-3 border border-slate-200 dark:border-slate-800 shadow-sm">
                    <div className="flex justify-center">
                      <SimpleFretboardDiagram chord={item.chord} />
                    </div>
                    <p className="text-xs text-amber-700 dark:text-amber-400 font-mono mt-2">{item.chordLabel}</p>
                  </div>
                </div>
              </div>
            ))}
