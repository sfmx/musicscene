...
const platformColors: Record<string, string> = {
  spotify: 'text-green-600 hover:bg-green-50',
  youtube: 'text-red-600 hover:bg-red-50',
  'apple-music': 'text-pink-600 hover:bg-pink-50',
  'ultimate-guitar': 'text-orange-600 hover:bg-orange-50',
  songsterr: 'text-indigo-600 hover:bg-indigo-50',
  musicnotes: 'text-blue-600 hover:bg-blue-50',
};
...
function LinkPill({ link }: { link: ExternalSongLink }) {
  return (
    <a
      ...
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-gray-200 text-xs font-medium transition-colors ${platformColors[link.icon]}`}
      title={link.label}
    >
...
export default function ExternalSongLinks({ title, artist }: ExternalSongLinksProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-gray-500">
      ...
      <span className="font-medium">Tabs & Sheet Music:</span>
      <div className="flex flex-wrap gap-1.5">
