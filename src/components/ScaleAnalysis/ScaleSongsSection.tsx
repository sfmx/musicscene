        ...
        {scaleData.famousSongs.map((song, index) => (
          <div key={index} className="bg-slate-50 dark:bg-slate-950/80 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
            <div className="flex justify-between items-start mb-3">
              <div>
                <h3 className="font-bold text-slate-900 dark:text-white text-base">{song.title}</h3>
                <p className="text-slate-600 dark:text-slate-400 text-xs">{song.artist}</p>
              </div>
              {song.difficulty && (
            ...
            <div className="space-y-2 text-xs">
              ...
              {song.genre && (
                <div className="flex justify-between py-1 border-b border-slate-200 dark:border-slate-800/80">
                  ...
                  <span className="text-slate-700 dark:text-slate-200">{song.genre}</span>
                </div>
              )}
              {song.usage && (
