      ...
      <div className="grid md:grid-cols-2 gap-6">
        {scaleData.genreApplications.map((genre, index) => (
          <div key={index} className="bg-slate-50 dark:bg-slate-950/80 rounded-xl p-5 border border-slate-200 dark:border-slate-800">
            <h3 className="font-bold text-slate-900 dark:text-white text-base mb-2">{genre.genre}</h3>
            <p className="text-slate-600 dark:text-slate-400 text-xs mb-4 leading-relaxed">{genre.usage}</p>
            
            {/* Characteristics */}
            <div className="mb-4">
              <h4 className="font-semibold text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider mb-2">Characteristics:</h4>
              <div className="flex flex-wrap gap-1.5">
                {genre.characteristics.map((characteristic, charIndex) => (
                  <span 
                    key={charIndex}
                    className="bg-white dark:bg-slate-900 border border-cyan-200 dark:border-cyan-500/30 text-cyan-700 dark:text-cyan-300 px-2 py-0.5 rounded text-xs"
                  >
                    {characteristic}
                  </span>
                ))}
            ...
            {/* Examples */}
            <div>
              <h4 className="font-semibold text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider mb-2">Examples:</h4>
